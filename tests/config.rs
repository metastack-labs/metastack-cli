...
"#,
    ...
    assert_eq!(
        ...
        "unknown agent command route key `backlogoops`; supported keys: backlog.spec, backlog.plan, backlog.improve, backlog.split, context.scan, context.reload, linear.issues.refine, agents.listen, agents.workflows.run, runtime.cron.prompt, merge.run"
    );
