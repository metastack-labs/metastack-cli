...
- Built-in `codex` and `claude` runs use one shared provider adapter path and one precedence model for provider/model/reasoning/transport resolution. Command output and dry-run paths should expose the resolved provider, model, reasoning, transport, route key, and config source whenever that launch path is part of the ticket scope.
